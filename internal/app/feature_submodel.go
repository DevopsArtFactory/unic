...
func (m *Model) featureSubmodels() []featureSubmodel {
	return []featureSubmodel{&m.ec2Browser, &m.ecs, &m.eks, &m.ecr, &m.fis, &m.vpc, &m.reachability, &m.cwMetrics, &m.cwAlarms, &m.cloudTrail, &m.cwLogs, &m.rds, &m.route53, &m.iam, &m.bedrock, &m.secrets, &m.security, &m.s3, &m.sqs, &m.elb, &m.ssmParams, &m.elasticache, &m.lambda, &m.inspector}
}

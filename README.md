...
| EventBridge Rules | `/` filter, `r` refresh, `Enter` rule detail, detail `↑`/`↓` scroll, `e` enable or `d` disable (type-to-confirm); AWS-managed rules are read-only |
...
The EventBridge Rules Browser lists rules from every event bus with disabled rules first, their schedule or event-pattern trigger, attached targets, and recent trigger activity. “Last Seen” is a best-effort view of the `AWS/Events` `TriggeredRules` CloudWatch metric over the previous seven days in 30-minute buckets; missing or delayed metrics are not a complete invocation audit, and missing `cloudwatch:GetMetricData` permission leaves rule and target browsing available with activity marked unavailable. Enabling or disabling a customer-managed rule requires typing its name and uses `events:EnableRule` or `events:DisableRule`; browsing requires `events:ListEventBuses`, `events:ListRules`, and `events:ListTargetsByRule`.

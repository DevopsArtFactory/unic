...
| API Gateway | HTTP/WebSocket API Browser |
| S3 | S3 Browser |
...
| API Gateway v2 | `/` filter APIs or routes, `r` refresh, `Enter` API/stage/route detail, detail `↑`/`↓` scroll and `PgUp`/`PgDn` page, route detail `y` copy integration target, `g` jump to a derived Lambda function |
...
The API Gateway v2 Browser lists HTTP and WebSocket APIs in the active region with protocol, default execute-api endpoint posture, endpoint, and creation time. API detail shows deployment and auto-deploy state for every stage, then opens a filterable route list with authorization and linked integration metadata. Route detail exposes the integration URI in copyable form and can jump to the Lambda Browser with a derived function-name filter. Stage, route, and integration collections paginate independently; a denied detail collection is reported inline while the API and other successful collections remain visible. This read-only flow requires `apigateway:GET` access to API Gateway v2 APIs, stages, routes, and integrations.
